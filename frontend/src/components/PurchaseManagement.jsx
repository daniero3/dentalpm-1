...
const PurchaseManagement = () => {
  ...
  const addItem = () => {
    setItems([...items, { product_id: '', qty: 1, unit_price_mga: 0 }]);
  };
  ...
  const updateItem = (index, field, value) => {
    ...
    if (field === 'product_id') {
      ...
      if (product) {
        newItems[index].unit_price_mga = parseFloat(product.unit_cost_mga) || 0;
      }
    }
  ...
  const calculateTotal = () => {
    return items.reduce((sum, item) => sum + (item.qty * item.unit_price_mga), 0);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {purchases.length} bon(s) de commande
          </p>
        </div>
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Créez un bon de commande pour un fournisseur
                </DialogDescription>
              </DialogHeader>
                ...
                {items.length > 0 && (
                  <div className="flex justify-end p-3 bg-gray-50 rounded-lg">
                    <span className="text-lg font-bold">
                      Total: {formatMoney(calculateTotal())} Ar
                    </span>
                  </div>
                )}
            ...
            <Card key={purchase.id} data-testid={`purchase-${purchase.id}`}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-lg font-bold">
                      {formatMoney(purchase.total_mga)} Ar
                    </span>
                    <div className="flex gap-2">
                      <Button 
                        variant="outline" 
                      ...
                      {purchase.status === 'DRAFT' && (
                        <Button 
                          ...
                          data-testid={`receive-${purchase.id}`}
                        >
