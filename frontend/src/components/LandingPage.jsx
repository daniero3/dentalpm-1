...
const GlobalCSS = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Bricolage+Grotesque:opsz,wght@12..96,300;12..96,400;12..96,500;12..96,600;12..96,700;12..96,800&display=swap');
    :root {
      --teal:#0D7A87; --teal-dk:#083D44; --teal-lt:#13A3B4;
      --ink:#0A0F14; --slate:#4A5568; --muted:#94A3B8;
      --border:#E8EDF2; --surface:#F7F9FC; --white:#FFFFFF;
      ...
      --sh-teal:0 8px 32px rgba(13,122,135,.28);
    }
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .feat-item{transition:all .25s ease;border-radius:var(--r12);}
    .feat-item:hover{background:var(--teal-pale,#F0FDFE)!important;transform:translateX(4px);}
...
/* ── Logo ─────────────────────────────────────────────────────────────────── */
const Logo = ({ size=36, dark=false }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
    <rect width="100" height="100" rx="18" fill={dark ? 'var(--teal-dk)' : 'var(--teal)'}/>
    ...
    <path d="M36 22C29 26,25 34,25 42C25 44,27 45,28 44C30 36,35 27,43 23C45 22,45 19,43 18C40 18,38 19,36 22Z" fill="white" opacity=".45"/>
...
/* ── FadeSlider ───────────────────────────────────────────────────────────── */
const FadeSlider = ({ images, height=480, interval=4500, radius='var(--r24)' }) => {
  ...
  return (
    <div style={{ position:'relative', height, borderRadius:radius, overflow:'hidden', boxShadow:'var(--sh3)' }}>
      <style>{`@keyframes sImgIn{from{opacity:0;transform:scale(1.04)}to{opacity:1;transform:scale(1)}}@keyframes sImgOut{from{opacity:1}to{opacity:0}}`}</style>
      {prev !== null && <img src={images[prev].src} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', animation:'sImgOut .8s ease both', zIndex:1 }}/>}
      <img key={cur} src={images[cur].src} alt={images[cur].alt} style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', animation:'sImgIn 1s cubic-bezier(.22,1,.36,1) both', zIndex:2 }}/>
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to bottom,transparent 55%,rgba(8,61,68,.55) 100%)', zIndex:3 }}/>
      <div style={{ position:'absolute', bottom:0, left:0, right:0, zIndex:4, padding:'18px 22px' }}>
        <p style={{ color:'rgba(255,255,255,.88)', fontSize:13, fontWeight:500, margin:0 }}>{images[cur].caption}</p>
      </div>
      <div style={{ position:'absolute', bottom:14, right:16, zIndex:5, display:'flex', gap:5 }}>
        {images.map((_,i) => <div key={i} onClick={() => setCur(i)} style={{ width:i===cur?20:6, height:6, borderRadius:99, background:i===cur?'#fff':'rgba(255,255,255,.4)', cursor:'pointer', transition:'all .4s' }}/>)}
      </div>
    </div>
  );
};
  ...
  <div style={{ overflow:'hidden', position:'relative' }}>
    <div style={{ display:'flex', gap:48, animation:'slide 18s linear infinite', width:'max-content' }}>
      {[...items, ...items].map((item, i) => (
        <div key={i} style={{ whiteSpace:'nowrap', fontSize:15, fontWeight:700, color:'var(--muted)', letterSpacing:.5 }}>{item}</div>
      ))}
    </div>
  </div>
);
    ...
    return (
      <svg width={64} height={64} viewBox="0 0 64 64">
        ...
        <circle cx={32} cy={32} r={r} fill="none" stroke={color} strokeWidth={7}
          ...
          transform="rotate(-90 32 32)" style={{ transition:'stroke-dasharray 1.2s' }}/>
        <text x={32} y={36} textAnchor="middle" fontSize={11} fontWeight={800} fill={color}>{pct}%</text>
      </svg>
    );
  };
  ...
  const screens = [
    ...
    <div key={0}>
      ...
      <svg width="100%" viewBox="0 0 200 52" style={{ overflow:'visible' }}>
        ...
        {(() => {
          const pts=[90,140,110,185,160,225,195,260];
          const W=200,H=48,mn=Math.min(...pts),mx=Math.max(...pts);
          const xs=pts.map((_,i)=>(i/(pts.length-1))*W);
          const ys=pts.map(p=>H-((p-mn)/(mx-mn||1))*(H-10)-5);
          const d=xs.map((x,i)=>`${i?'L':'M'}${x},${ys[i]}`).join(' ');
          return (<>
            <path d={`${d} L${W},${H} L0,${H}Z`} fill="url(#dg)"/>
            <path d={d} fill="none" stroke="#0D7A87" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"/>
            {xs.map((x,i)=><circle key={i} cx={x} cy={ys[i]} r={3} fill="#0D7A87" stroke="#fff" strokeWidth={1.5}/>)}
          </>);
        })()}
      </svg>
    </div>,
    ...
    <div key={1}>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:11 }}>
        <span style={{ fontWeight:700, fontSize:13, color:'#0F172A' }}>Patients du jour</span>
        <span style={{ background:'#F0FDFE', color:'#0D7A87', borderRadius:99, padding:'2px 10px', fontSize:11, fontWeight:700 }}>247</span>
      </div>
      {[{n:'Rakoto Jean',h:'09:00',s:'Confirmé',c:'#10B981',b:'#D1FAE5'},{n:'Rasoa Marie',h:'10:30',s:'En attente',c:'#F59E0B',b:'#FEF3C7'},{n:'Andry Paul',h:'11:00',s:'Confirmé',c:'#10B981',b:'#D1FAE5'},{n:'Hanta Elisa',h:'14:00',s:'Nouveau',c:'#7C3AED',b:'#EDE9FE'}].map((p,i)=>(
        <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 10px', background:'#F8FAFC', borderRadius:10, marginBottom:5, border:'1px solid #F1F5F9' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ width:28,height:28,borderRadius:'50%',background:'linear-gradient(135deg,#0D7A87,#13A3B4)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:11,fontWeight:800 }}>{p.n[0]}</div>
            <div><div style={{fontWeight:700,fontSize:12,color:'#0F172A'}}>{p.n}</div><div style={{fontSize:10,color:'#94A3B8'}}>RDV {p.h}</div></div>
          </div>
    ...
    <div key={2}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
        <span style={{ fontWeight:700, fontSize:13 }}>Rapports financiers</span>
        <span style={{ background:'#D1FAE5', color:'#065F46', borderRadius:99, padding:'2px 9px', fontSize:11, fontWeight:700 }}>+18% ce mois</span>
      </div>
      <div style={{ display:'flex', justifyContent:'space-around', marginBottom:14 }}>
        <Donut pct={78} color="#0D7A87"/><Donut pct={92} color="#10B981"/><Donut pct={65} color="#F59E0B"/>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
        {[{l:'Occupation',v:'78%'},{l:'Paiements',v:'92%'},{l:'Objectif',v:'65%'}].map((s,i)=>(
          <div key={i} style={{ textAlign:'center', background:'#F8FAFC', borderRadius:8, padding:'8px 4px', border:'1px solid #E2E8F0' }}>
            <div style={{ fontWeight:800, fontSize:14, color:'#0F172A' }}>{s.v}</div>
    ...
    <div key={3}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:11 }}>
        <span style={{ fontWeight:700, fontSize:13 }}>Agenda du jour</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'36px 1fr', gap:'3px 8px' }}>
        {[{h:'08:30',n:'Détartrage',d:'Dr. Rakoto',c:'#0D7A87',bg:'#F0FDFE',dur:'45min'},{h:'09:15',n:'Carie M16',d:'Dr. Rasoa',c:'#7C3AED',bg:'#F5F3FF',dur:'30min'},{h:'10:00',n:'Couronne',d:'Dr. Rakoto',c:'#F59E0B',bg:'#FFFBEB',dur:'90min'},{h:'11:30',n:'Pause',d:'',c:'#CBD5E1',bg:'#F8FAFC',dur:'30min'},{h:'14:00',n:'Extraction',d:'Dr. Andry',c:'#EF4444',bg:'#FFF1F2',dur:'45min'}].map((r,i)=>(
          <React.Fragment key={i}>
            ...
            <div style={{background:r.bg,borderLeft:`3px solid ${r.c}`,borderRadius:'0 8px 8px 0',padding:'5px 9px',marginBottom:4}}>
              <div style={{fontSize:11,fontWeight:700,color:r.c}}>{r.n}</div>
              <div style={{fontSize:10,color:'#94A3B8'}}>{r.d}{r.d?' · ':''}{r.dur}</div>
            </div>
          </React.Fragment>
        ))}
      </div>
    ...
    <div key={4}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:11 }}>
        <span style={{ fontWeight:700, fontSize:13 }}>Odontogramme FDI</span>
  ...
  return (
    <div style={{ background:'#fff', borderRadius:20, padding:'16px 18px', boxShadow:'0 24px 64px rgba(0,0,0,.14)', border:'1px solid #E2E8F0', display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:14, paddingBottom:12, borderBottom:'1px solid #F1F5F9' }}>
        ...
        <div style={{ flex:1, background:'#F8FAFC', borderRadius:6, padding:'4px 12px', marginLeft:10, display:'flex', alignItems:'center', gap:6 }}>
      ...
      <div style={{ display:'flex', gap:3, marginBottom:13, flexWrap:'wrap' }}>
        {tabs.map((t,i) => (
          <button key={i} onClick={() => setTab(i)} style={{ padding:'4px 9px', borderRadius:7, border:'none', cursor:'pointer', background:tab===i?'#0D7A87':'#F1F5F9', color:tab===i?'#fff':'#64748B', fontSize:10, fontWeight:700, transition:'all .2s' }}>{t}</button>
        ))}
      </div>
...
const Counter = ({ end, suffix='', label, sub }) => {
  ...
  return (
    <div ref={ref} className="stat-card" style={{ background:'#fff', borderRadius:20, padding:'28px 24px', border:'1px solid var(--border)', boxShadow:'var(--sh1)', animation:'countUp .6s ease both' }}>
      <div style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:44, color:'var(--teal)', lineHeight:1, marginBottom:6 }}>
        {n}{suffix}
      </div>
      <div style={{ fontFamily:'Bricolage Grotesque', fontWeight:700, fontSize:16, color:'var(--ink)', marginBottom:4 }}>{label}</div>
...
const PLANS = [
  { name:'ESSENTIAL', price:'149 000', popular:false,
    desc:'Idéal pour les cabinets solo',
  ...
  { name:'PRO', price:'199 000', popular:true,
    desc:'Le plus choisi par nos clients',
...
const InscriptionModal = ({ show, plan, onClose, navigate }) => {
  ...
  const inp = { width:'100%', padding:'12px 14px', borderRadius:12, border:'1.5px solid var(--border)', fontSize:14, fontFamily:'Inter,sans-serif', outline:'none', transition:'border-color .2s, box-shadow .2s' };
  ...
  if (!show) return null;
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(10,15,20,.75)', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16, animation:'fadeIn .2s ease' }}>
      <div style={{ background:'#fff', borderRadius:28, padding:'40px 36px', maxWidth:500, width:'100%', maxHeight:'92vh', overflowY:'auto', position:'relative', animation:'scaleIn .3s cubic-bezier(.22,1,.36,1)' }}>
        <button onClick={onClose} style={{ position:'absolute', top:16, right:16, width:34, height:34, borderRadius:'50%', background:'var(--surface)', border:'none', cursor:'pointer', fontSize:18, color:'var(--muted)' }}>✕</button>
        {!done ? (
          <>
            ...
            <h2 style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:22, color:'var(--ink)', marginBottom:6 }}>
              {step===1 ? '🚀 Démarrer votre essai gratuit' : '💳 Modalités de paiement'}
            </h2>
            {plan && <div style={{ background:'#F0FDFE', border:'1.5px solid var(--teal)', borderRadius:12, padding:'10px 16px', margin:'12px 0 20px', display:'flex', justifyContent:'space-between' }}>
            ...
            {step===1 && (
              <div>
                ...
                <button className="btn-main" disabled={!form.cabinet||!form.email||!form.phone||!form.city} onClick={()=>setStep(2)}
                  style={{width:'100%',padding:'14px',borderRadius:13,background:'var(--teal)',color:'#fff',fontWeight:700,fontSize:15,border:'none',cursor:'pointer',opacity:(!form.cabinet||!form.email||!form.phone||!form.city)?.5:1}}>
                  Continuer →
                </button>
              </div>
            )}
            {step===2 && (
              <div>
                <p style={{color:'var(--slate)',fontSize:14,lineHeight:1.7,marginBottom:16}}>Votre <strong>essai de 7 jours</strong> commence immédiatement. À la fin, payez par :</p>
                ...
                <button className="btn-main" onClick={submit} disabled={loading}
                  style={{width:'100%',marginTop:16,padding:'14px',borderRadius:13,background:'var(--teal)',color:'#fff',fontWeight:700,fontSize:15,border:'none',cursor:'pointer',opacity:loading?.6:1}}>
          ...
          <div style={{textAlign:'center',padding:'12px 0'}}>
            ...
            <h2 style={{fontFamily:'Bricolage Grotesque',fontWeight:800,fontSize:24,color:'var(--ink)',marginBottom:8}}>Bienvenue sur DPM !</h2>
            <p style={{color:'var(--slate)',lineHeight:1.8,marginBottom:22}}>Cabinet <strong>{form.cabinet}</strong> créé !<br/>Identifiants envoyés à <strong>{form.email}</strong></p>
            <div style={{background:'#F0FDFE',border:'1.5px solid var(--teal)',borderRadius:14,padding:'14px 18px',marginBottom:22,textAlign:'left'}}>
            ...
            <button className="btn-main" onClick={()=>navigate('/login')}
              style={{width:'100%',padding:'14px',borderRadius:13,background:'var(--teal)',color:'#fff',fontWeight:700,fontSize:15,border:'none',cursor:'pointer'}}>
...
export default function LandingPage() {
  ...
  const ST = ({ tag, title, sub, light=false }) => (
    <div style={{ marginBottom:56 }}>
      <span className="tag" style={{ marginBottom:14, display:'inline-flex', color: light?'#7DD3DA':'var(--teal)', background:light?'rgba(255,255,255,.1)':'#F0FDFE', borderColor:light?'rgba(255,255,255,.2)':'#7DD3DA' }}>{tag}</span>
      <h2 style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:44, color:light?'#fff':'var(--ink)', lineHeight:1.15, marginBottom:14 }}>{title}</h2>
      {sub && <p style={{ fontSize:17, color:light?'rgba(255,255,255,.68)':'var(--slate)', maxWidth:560, lineHeight:1.75 }}>{sub}</p>}
    </div>
  );

  return (
    <div style={{ fontFamily:"'Inter', sans-serif", background:'var(--white)', minHeight:'100vh', overflowX:'hidden' }}>
      <GlobalCSS />
      ...
      <nav style={{
        ...
        borderBottom: scrolled ? '1px solid rgba(232,237,242,.8)' : 'none',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <Logo size={40}/>
          <span style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:22, color:scrolled?'var(--ink)':'#fff' }}>DPM</span>
          <span style={{ fontSize:11, fontWeight:600, padding:'3px 10px', borderRadius:99, background:scrolled?'var(--surface)':'rgba(255,255,255,.12)', color:scrolled?'var(--muted)':'rgba(255,255,255,.7)', border:`1px solid ${scrolled?'var(--border)':'rgba(255,255,255,.2)'}` }}>Madagascar</span>
        </div>
        <div style={{ display:'flex', gap:2, alignItems:'center' }}>
          {[['#services','Fonctionnalités'],['#pourquoi','Avantages'],['#tarifs','Tarifs'],['#faq','FAQ'],['#contact','Contact']].map(([href,label]) => (
            <a key={href} href={href} className="nav-link" style={{ padding:'7px 13px', color:scrolled?'var(--slate)':'rgba(255,255,255,.8)', fontWeight:500, fontSize:14, borderRadius:9 }}>{label}</a>
          ))}
          <button onClick={() => navigate('/login')} style={{ marginLeft:8, padding:'8px 18px', borderRadius:10, border:`1.5px solid ${scrolled?'var(--border)':'rgba(255,255,255,.3)'}`, background:'transparent', color:scrolled?'var(--ink)':'#fff', fontWeight:600, fontSize:14, cursor:'pointer', transition:'all .2s' }}>Connexion</button>
          <button onClick={() => open(PLANS[1])} className="btn-main"
            style={{ marginLeft:6, padding:'9px 20px', borderRadius:10, background:'var(--teal)', color:'#fff', fontWeight:700, fontSize:14, border:'none', cursor:'pointer', boxShadow:'var(--sh-teal)' }}>
            Essai gratuit 7j 🚀
          </button>
        </div>
      </nav>
      ...
      {/* ══ HERO ══════════════════════════════════════════════════════════ */}
      <section style={{
        background:'linear-gradient(135deg,var(--teal-dk) 0%,#0A5F6A 50%,var(--teal) 100%)',
        minHeight:'100vh', display:'flex', alignItems:'center',
        padding:'120px 56px 80px', position:'relative', overflow:'hidden',
      }}>
        {/* Orbs */}
        <div style={{ position:'absolute', top:'10%', left:'3%', width:360, height:360, borderRadius:'50%', background:'radial-gradient(circle,rgba(13,122,135,.45),transparent 70%)', filter:'blur(60px)', animation:'float 12s ease-in-out infinite', pointerEvents:'none' }}/>
        <div style={{ position:'absolute', bottom:'10%', right:'3%', width:280, height:280, borderRadius:'50%', background:'radial-gradient(circle,rgba(125,211,218,.3),transparent 70%)', filter:'blur(50px)', animation:'float 16s ease-in-out infinite reverse', pointerEvents:'none' }}/>
        {/* Grid */}
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', width:'100%', display:'grid', gridTemplateColumns:'1fr 1fr', gap:64, alignItems:'center' }}>
          <div>
            <div className="au0" style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(255,255,255,.1)', border:'1px solid rgba(255,255,255,.18)', borderRadius:99, padding:'7px 18px', marginBottom:28 }}>
              ...
              <span style={{ fontSize:13, color:'rgba(255,255,255,.9)', fontWeight:500 }}>🇲🇬 N°1 des logiciels dentaires à Madagascar</span>
            </div>
            ...
            <div className="au2" style={{ height:40, marginBottom:20, display:'flex', alignItems:'center' }}>
              <span style={{ fontSize:19, color:'rgba(255,255,255,.65)' }}>
            ...
            <p className="au2" style={{ fontSize:17, color:'rgba(255,255,255,.68)', lineHeight:1.78, marginBottom:36, maxWidth:500 }}>
            ...
            <div className="au3" style={{ display:'flex', gap:14, flexWrap:'wrap', marginBottom:32 }}>
              <button onClick={() => open(PLANS[1])} className="btn-main"
                style={{ padding:'16px 32px', borderRadius:13, background:'#fff', color:'var(--teal)', fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:16, border:'none', cursor:'pointer', boxShadow:'0 12px 40px rgba(0,0,0,.2)' }}>
                Essayer gratuitement — 7 jours ✨
              </button>
              <a href="#tarifs" style={{ padding:'16px 28px', borderRadius:13, background:'rgba(255,255,255,.1)', color:'#fff', fontWeight:600, fontSize:16, border:'1px solid rgba(255,255,255,.22)', textDecoration:'none', display:'inline-flex', alignItems:'center', transition:'all .2s' }}>
                Voir les tarifs →
              </a>
            </div>
          ...
          <div className="au3" style={{ position:'relative' }}>
            <div style={{ height:490 }}><DashMockup /></div>
            <div style={{ position:'absolute', bottom:-16, left:-20, background:'#fff', borderRadius:16, padding:'14px 18px', boxShadow:'var(--sh3)', display:'flex', alignItems:'center', gap:10, animation:'float 4s ease-in-out infinite', zIndex:10 }}>
              <span style={{ fontSize:26 }}>🦷</span>
              <div>
                <div style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, color:'var(--teal)', fontSize:15 }}>+50 cabinets</div>
                <div style={{ color:'var(--muted)', fontSize:11 }}>font confiance à DPM</div>
              </div>
            </div>
            <div style={{ position:'absolute', top:-14, right:-16, background:'linear-gradient(135deg,var(--teal),var(--teal-lt))', borderRadius:14, padding:'11px 16px', boxShadow:'var(--sh-teal)', animation:'float 5.5s ease-in-out infinite reverse', zIndex:10 }}>
        ...
        {/* Scroll */}
        <div style={{ position:'absolute', bottom:28, left:'50%', transform:'translateX(-50%)', display:'flex', flexDirection:'column', alignItems:'center', gap:5, opacity:.45, pointerEvents:'none' }}>
      ...
      {/* ══ STATS ═════════════════════════════════════════════════════════ */}
      <section style={{ background:'var(--surface)', padding:'60px 56px', borderBottom:'1px solid var(--border)' }}>
      ...
      {/* ══ LOGOS DÉFILANT ════════════════════════════════════════════════ */}
      <section style={{ padding:'32px 0', borderBottom:'1px solid var(--border)', overflow:'hidden' }}>
      ...
      {/* ══ HERO CENTRAL (inspiration Matisse) ═══════════════════════════ */}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          {FEATURES_LIST.map((f, i) => (
            <a key={i} href="#services" className="feat-item sr" onClick={e => e.preventDefault()}
              style={{ display:'flex', gap:16, alignItems:'flex-start', padding:'22px 24px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, textDecoration:'none', transitionDelay:`${i*.05}s` }}>
              <div style={{ width:46, height:46, borderRadius:14, background:`${f.color}15`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, flexShrink:0 }}>{f.icon}</div>
              <div>
                <span style={{ fontSize:11, fontWeight:700, color:f.color, textTransform:'uppercase', letterSpacing:1.5, display:'block', marginBottom:4 }}>{f.tag}</span>
                <div style={{ fontFamily:'Bricolage Grotesque', fontWeight:700, fontSize:16, color:'var(--ink)', marginBottom:5 }}>{f.title}</div>
                <div style={{ fontSize:14, color:'var(--slate)', lineHeight:1.6 }}>{f.desc}</div>
              </div>
            </a>
          ))}
        </div>
      ...
      {/* ══ SECTION SERVICES ══════════════════════════════════════════════ */}
      <section id="services" style={{ background:'var(--surface)', padding:'100px 56px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              {['Odontogramme FDI complet avec historique par dent','Facturation MGA avec support MVola et Orange Money','Ordonnances et prescriptions format standard Madagascar','Gestion laboratoire prothèses et implants','Inventaire matériel avec alertes stock automatiques'].map((item, i) => (
                <div key={i} style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                  <div style={{ width:24, height:24, borderRadius:'50%', background:'#F0FDFE', border:'2px solid var(--teal)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, marginTop:1 }}>
                    <span style={{ color:'var(--teal)', fontWeight:800, fontSize:11 }}>✓</span>
                  </div>
                  <span style={{ color:'var(--slate)', fontSize:15, lineHeight:1.6 }}>{item}</span>
                </div>
              ))}
            </div>
            <button onClick={() => open(PLANS[1])} className="btn-main"
              style={{ marginTop:30, padding:'13px 26px', borderRadius:12, background:'var(--teal)', color:'#fff', fontWeight:700, fontSize:15, border:'none', cursor:'pointer' }}>
              Essayer gratuitement →
            </button>
          </div>
          <div className="sr" style={{ transitionDelay:'.15s' }}>
            <FadeSlider images={IMGS_SERVICES} height={460} interval={4200}/>
          </div>
        </div>
      </section>
      ...
      <section style={{ background:'var(--ink)', padding:'80px 56px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:56, alignItems:'center', position:'relative' }}>
          <div className="sr">
            <span style={{ fontSize:11, fontWeight:700, color:'#7DD3DA', letterSpacing:3, textTransform:'uppercase', display:'block', marginBottom:14 }}>🚀 REJOIGNEZ DPM</span>
            <h2 style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:42, color:'#fff', lineHeight:1.15, marginBottom:16 }}>
              Révolutionnez la gestion de votre cabinet
            </h2>
            <p style={{ color:'rgba(255,255,255,.65)', fontSize:16, lineHeight:1.75, marginBottom:28 }}>
              Transformez votre cabinet dentaire en quelques minutes. Interface moderne, fonctionnalités complètes, sécurité maximale.
            </p>
            {['✓ Démonstration en 7 jours gratuits','✓ Prise en main en 30 minutes','✓ Support technique prioritaire'].map(item=>(
              <div key={item} style={{ fontSize:15, color:'rgba(255,255,255,.75)', marginBottom:8 }}>{item}</div>
            ))}
          </div>
          <div className="sr" style={{ transitionDelay:'.15s', display:'flex', flexDirection:'column', gap:14 }}>
            <button onClick={() => open(PLANS[1])} className="btn-main"
              style={{ padding:'18px 32px', borderRadius:13, background:'#fff', color:'var(--teal)', fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:17, border:'none', cursor:'pointer' }}>
              Commencer maintenant →
            </button>
            <a href="#tarifs" style={{ padding:'16px 32px', borderRadius:13, background:'transparent', color:'rgba(255,255,255,.7)', fontWeight:600, fontSize:16, border:'1px solid rgba(255,255,255,.2)', textDecoration:'none', textAlign:'center', transition:'all .2s' }}>
              Voir les tarifs
            </a>
            <div style={{ display:'flex', gap:16, justifyContent:'center', flexWrap:'wrap' }}>
              {[{v:'⚡',l:'5 minutes'},{v:'🇲🇬',l:'Made in Mada'},{v:'🔒',l:'100% sécurisé'}].map(b=>(
                <div key={b.l} style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:11, color:'rgba(255,255,255,.45)', marginTop:3 }}>{b.l}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ══ POURQUOI ══════════════════════════════════════════════════════ */}
      <section id="pourquoi" style={{ padding:'100px 56px', maxWidth:1200, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20 }}>
          {[
            ...
            <div key={i} className="sr plan-hover" style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:20, padding:'28px 24px', transitionDelay:`${i*.07}s` }}>
              <div style={{ width:52,height:52,borderRadius:14,background:`${a.c}12`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:26,marginBottom:16 }}>{a.icon}</div>
              <h3 style={{ fontFamily:'Bricolage Grotesque', fontWeight:700, fontSize:18, color:'var(--ink)', marginBottom:8 }}>{a.t}</h3>
              <p style={{ color:'var(--slate)', fontSize:14, lineHeight:1.7 }}>{a.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background:'var(--surface)', padding:'100px 56px' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:22, marginBottom:48 }}>
            {TEMOIGNAGES.map((t,i)=>(
              <div key={i} className="sr plan-hover" style={{ background:'#fff', borderRadius:22, padding:'28px 26px', border:'1px solid var(--border)', boxShadow:'var(--sh1)', transitionDelay:`${i*.1}s` }}>
                <div style={{ display:'flex', gap:2, marginBottom:14 }}>{Array(5).fill(0).map((_,j)=><span key={j} style={{color:'#F59E0B',fontSize:18}}>★</span>)}</div>
                <p style={{ color:'var(--slate)', fontSize:15, lineHeight:1.8, marginBottom:20, fontStyle:'italic' }}>&ldquo;{t.txt}&rdquo;</p>
                <div style={{ display:'flex', alignItems:'center', gap:12, paddingTop:18, borderTop:'1px solid var(--border)' }}>
                  <div style={{ width:44,height:44,borderRadius:'50%',background:'linear-gradient(135deg,var(--teal),var(--teal-lt))',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontFamily:'Bricolage Grotesque',fontWeight:800,fontSize:17 }}>{t.nom.split(' ').pop()[0]}</div>
                  <div>
                    <div style={{ fontFamily:'Bricolage Grotesque', fontWeight:700, color:'var(--ink)', fontSize:14 }}>{t.nom}</div>
          ...
          <div className="sr">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:28 }}>
              {[{v:'2024',l:'Année de création'},{v:'Tana',l:'Basé à Antananarivo'},{v:'🇲🇬',l:'Made in Madagascar'},{v:'24/7',l:'Support disponible'}].map((s,i)=>(
                <div key={i} style={{ background:'var(--surface)', borderRadius:14, padding:'18px 20px', border:'1px solid var(--border)', textAlign:'center' }}>
                  <div style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:26, color:'var(--teal)' }}>{s.v}</div>
                  <div style={{ color:'var(--muted)', fontSize:13, marginTop:4 }}>{s.l}</div>
                </div>
              ))}
            </div>
            <button onClick={() => open(PLANS[1])} className="btn-main"
              style={{ padding:'13px 26px', borderRadius:12, background:'var(--teal)', color:'#fff', fontWeight:700, fontSize:15, border:'none', cursor:'pointer' }}>
              Rejoindre DPM →
            </button>
          </div>
          <div className="sr" style={{ transitionDelay:'.15s' }}>
            <FadeSlider images={IMGS_ABOUT} height={500} interval={5000}/>
          </div>
        </div>
      </section>

      {/* ══ TARIFS ════════════════════════════════════════════════════════ */}
      <section id="tarifs" style={{ background:'var(--surface)', padding:'100px 56px' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <div className="sr" style={{ textAlign:'center', marginBottom:60 }}>
            ...
            <p style={{ color:'var(--teal)', fontWeight:600, fontSize:14, marginTop:-30 }}>💳 MVola · Orange Money · Airtel Money · Virement BNI</p>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20, alignItems:'start' }}>
            {PLANS.map((plan,i)=>(
              <div key={plan.name} className={`plan-hover sr`} style={{
                background:'#fff', borderRadius:22, padding:'32px 26px',
                border: plan.popular ? '2px solid var(--teal)' : '1px solid var(--border)',
                boxShadow: plan.popular ? 'var(--sh-teal)' : 'var(--sh1)',
                position:'relative', transitionDelay:`${i*.1}s`,
              }}>
                {plan.popular && <div style={{ position:'absolute', top:-13, left:'50%', transform:'translateX(-50%)', background:'var(--teal)', color:'#fff', padding:'4px 16px', borderRadius:99, fontSize:12, fontWeight:800, whiteSpace:'nowrap' }}>⭐ Le plus populaire</div>}
                <div style={{ marginBottom:6 }}>
                  <span style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:18, color:'var(--ink)' }}>{plan.name}</span>
                </div>
                <div style={{ fontSize:13, color:'var(--muted)', marginBottom:18 }}>{plan.desc}</div>
                <div style={{ marginBottom:22 }}>
                  <span style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:40, color:'var(--teal)' }}>{plan.price}</span>
                  <span style={{ color:'var(--muted)', fontSize:14 }}> Ar/mois</span>
                </div>
                <div style={{ height:1, background:'var(--border)', marginBottom:20 }}/>
                <ul style={{ listStyle:'none', padding:0, marginBottom:26 }}>
                  {plan.features.map((f,j)=>(
                    <li key={j} style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:9, color:'var(--slate)', fontSize:14 }}>
                      <span style={{ color:'var(--teal)', fontWeight:800, flexShrink:0 }}>✓</span>{f}
                    </li>
                  ))}
                </ul>
                <button onClick={() => open(plan)} className="btn-main"
                  style={{ width:'100%', padding:'13px', borderRadius:12, background:plan.popular?'var(--teal)':'transparent', color:plan.popular?'#fff':'var(--teal)', fontWeight:700, fontSize:15, border:`2px solid var(--teal)`, cursor:'pointer', transition:'all .2s' }}>
      ...
      {/* ══ FAQ ═══════════════════════════════════════════════════════════ */}
      <section id="faq" style={{ padding:'100px 56px', maxWidth:800, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {FAQS.map((faq,i)=>(
            <div key={i} className="faq-item sr" onClick={()=>setOpenFaq(openFaq===i?null:i)}
              style={{ background:'#fff', border:`1px solid ${openFaq===i?'var(--teal)':'var(--border)'}`, borderRadius:14, padding:'20px 24px', transitionDelay:`${i*.04}s` }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:16 }}>
                <p style={{ fontFamily:'Bricolage Grotesque', fontWeight:700, fontSize:15, color:'var(--ink)', margin:0 }}>{faq.q}</p>
              ...
              {openFaq===i && <p style={{ color:'var(--slate)', fontSize:14, lineHeight:1.8, marginTop:12, animation:'fadeIn .3s ease' }}>{faq.a}</p>}
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ══ CONTACT ═══════════════════════════════════════════════════════ */}
      <section id="contact" style={{ background:'var(--ink)', padding:'100px 56px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:72, position:'relative', alignItems:'start' }}>
          <div className="sr">
            <span style={{ fontSize:11, fontWeight:700, color:'#7DD3DA', letterSpacing:3, textTransform:'uppercase', display:'block', marginBottom:14 }}>📨 CONTACT</span>
            <h2 style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:42, color:'#fff', marginBottom:14, lineHeight:1.2 }}>Parlons de votre cabinet</h2>
            <p style={{ color:'rgba(255,255,255,.65)', fontSize:16, lineHeight:1.78, marginBottom:40 }}>Notre équipe à Antananarivo est disponible pour répondre à toutes vos questions.</p>
            {[
              ...
              <div key={i} style={{ display:'flex', gap:16, alignItems:'flex-start', marginBottom:22 }}>
                <div style={{ width:46,height:46,background:'rgba(255,255,255,.08)',borderRadius:13,display:'flex',alignItems:'center',justifyContent:'center',fontSize:21,flexShrink:0 }}>{c.icon}</div>
                <div>
                  <div style={{ color:'rgba(255,255,255,.45)', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1.5, marginBottom:4 }}>{c.label}</div>
                  {c.href?<a href={c.href} style={{color:'#fff',fontWeight:600,fontSize:16,textDecoration:'none'}}>{c.val}</a>:<div style={{color:'#fff',fontWeight:600,fontSize:16}}>{c.val}</div>}
                </div>
              </div>
            ))}
            <div style={{ marginTop:36, padding:'22px 24px', background:'rgba(255,255,255,.07)', borderRadius:16, border:'1px solid rgba(255,255,255,.12)' }}>
              <p style={{ color:'rgba(255,255,255,.5)', fontSize:12, fontWeight:700, textTransform:'uppercase', letterSpacing:1.5, marginBottom:12 }}>PRÊT À COMMENCER ?</p>
              <button onClick={() => open(PLANS[1])} className="btn-main"
                style={{ padding:'13px 24px', borderRadius:12, background:'#fff', color:'var(--teal)', fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:15, border:'none', cursor:'pointer' }}>
          ...
          <div className="sr" style={{ transitionDelay:'.15s' }}>
            {!contactSent?(
              <div style={{ background:'rgba(255,255,255,.07)', borderRadius:22, padding:'34px 30px', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,.1)' }}>
                <h3 style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:22, color:'#fff', marginBottom:22 }}>Envoyer un message</h3>
                {[{l:'Votre nom',n:'nom',ph:'Dr. Rakoto Jean',t:'text'},{l:'Email',n:'email',ph:'contact@cabinet.mg',t:'email'}].map(f=>(
                  <div key={f.n} style={{ marginBottom:14 }}>
                    ...
                    <input type={f.t} placeholder={f.ph} value={contact[f.n]} onChange={e=>setContact(p=>({...p,[f.n]:e.target.value}))}
                      style={{ width:'100%', padding:'12px 14px', borderRadius:11, border:'1.5px solid rgba(255,255,255,.15)', background:'rgba(255,255,255,.07)', color:'#fff', fontSize:14, fontFamily:'Inter,sans-serif', outline:'none', transition:'border-color .2s' }}
                ...
                <div style={{ marginBottom:20 }}>
                  ...
                  <textarea rows={5} placeholder="Décrivez votre besoin..." value={contact.message} onChange={e=>setContact(p=>({...p,message:e.target.value}))}
                    style={{ width:'100%', padding:'12px 14px', borderRadius:11, border:'1.5px solid rgba(255,255,255,.15)', background:'rgba(255,255,255,.07)', color:'#fff', fontSize:14, fontFamily:'Inter,sans-serif', outline:'none', resize:'vertical', transition:'border-color .2s' }}
                    onFocus={e=>e.target.style.borderColor='rgba(255,255,255,.45)'}
                    onBlur={e=>e.target.style.borderColor='rgba(255,255,255,.15)'}/>
                </div>
              ...
              <div style={{ background:'rgba(255,255,255,.07)', borderRadius:22, padding:'56px 32px', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,.1)', textAlign:'center' }}>
                <div style={{ fontSize:60, marginBottom:16, animation:'float 3s ease-in-out infinite' }}>✅</div>
                <h3 style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, fontSize:24, color:'#fff', marginBottom:10 }}>Message envoyé !</h3>
                <p style={{ color:'rgba(255,255,255,.65)', lineHeight:1.75 }}>Merci {contact.nom} !<br/>Nous vous répondrons à <strong style={{color:'#fff'}}>{contact.email}</strong> sous 24h.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ══ FOOTER ════════════════════════════════════════════════════════ */}
      <footer style={{ background:'#03080B', padding:'28px 56px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:14 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <Logo size={34}/>
          <span style={{ fontFamily:'Bricolage Grotesque', fontWeight:800, color:'rgba(255,255,255,.8)', fontSize:16 }}>DPM Madagascar</span>
        </div>
        <div style={{ display:'flex', gap:18 }}>
          {[['#services','Fonctionnalités'],['#pourquoi','Avantages'],['#tarifs','Tarifs'],['#faq','FAQ'],['#contact','Contact']].map(([h,l])=>(
        ...
        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
          ...
          <button onClick={() => navigate('/login')} style={{ padding:'6px 14px', borderRadius:8, border:'1px solid rgba(255,255,255,.15)', background:'transparent', color:'rgba(255,255,255,.5)', cursor:'pointer', fontSize:13, fontWeight:600, transition:'all .2s' }}>Connexion</button>
        </div>
      </footer>
