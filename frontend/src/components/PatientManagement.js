...
const labelStyle = { display: 'block', marginBottom: 6, fontSize: 13, fontWeight: 600, color: '#475569', fontFamily: 'Plus Jakarta Sans, sans-serif' };
...
const PatientManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (apiError?.details && Array.isArray(apiError.details)) {
        toast.error(apiError.details.map(function(d) { return d.msg || d.message; }).join(', '));
      } else {
  ...
  const setField = (field) => (e) => setFormData(function(f) { return Object.assign({}, f, { [field]: e.target.value }); });
  ...
  const ActionBtn = ({ to, icon: Icon, title }) => (
    <Link to={to} title={title}>
      <button
        ...
        onMouseEnter={function(e) { e.currentTarget.style.borderColor = '#0D7A87'; e.currentTarget.style.color = '#0D7A87'; e.currentTarget.style.background = '#F0F7F8'; }}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{
        ...
        <Dialog open={isDialogOpen} onOpenChange={function(open) { setIsDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild>
            <button
              ...
              onMouseEnter={function(e) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(13,122,135,0.4)'; }}
              onMouseLeave={function(e) { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 2px 12px rgba(13,122,135,0.3)'; }}
            >
          ...
          <DialogContent style={{ maxWidth: 640, maxHeight: '90vh', overflowY: 'auto', borderRadius: 16 }}>
            <DialogHeader>
              <DialogTitle style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 18, fontWeight: 800 }}>
            ...
            <form onSubmit={handleSubmit} style={{ marginTop: 8 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <input className={inputClass} value={formData.first_name} onChange={setField('first_name')} required placeholder="Prénom" />
                </div>
                <div>
                  <label style={labelStyle}>Nom *</label>
                  <input className={inputClass} value={formData.last_name} onChange={setField('last_name')} required placeholder="Nom" />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <input className={inputClass} type="date" value={formData.date_of_birth} onChange={setField('date_of_birth')} required />
                </div>
                <div>
                  <label style={labelStyle}>Sexe *</label>
                  <select className={inputClass} value={formData.gender} onChange={setField('gender')} required>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <input className={inputClass} value={formData.phone_primary} onChange={setField('phone_primary')} required placeholder="+261 32 00 000 00" data-testid="phone-input" />
                </div>
                <div>
                  <label style={labelStyle}>Email</label>
                  <input className={inputClass} type="email" value={formData.email} onChange={setField('email')} placeholder="patient@email.com" />
                </div>
              </div>

              <div style={fieldStyle}>
                <label style={labelStyle}>Adresse</label>
                <textarea className={inputClass} value={formData.address} onChange={setField('address')} placeholder="Adresse complète" rows={2} style={{ minHeight: 64, resize: 'vertical' }} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div>
                  <label style={labelStyle}>Contact d'urgence</label>
                  <input className={inputClass} value={formData.emergency_contact_name} onChange={setField('emergency_contact_name')} placeholder="Nom (optionnel)" />
                </div>
                <div>
                  <label style={labelStyle}>Tél. d'urgence</label>
                  <input className={inputClass} value={formData.emergency_contact_phone} onChange={setField('emergency_contact_phone')} placeholder="+261 32 (optionnel)" />
                </div>
              </div>

              <div style={fieldStyle}>
                <label style={labelStyle}>Antécédents médicaux</label>
                <textarea className={inputClass} value={formData.medical_history} onChange={setField('medical_history')} placeholder="Antécédents médicaux" rows={3} style={{ minHeight: 72, resize: 'vertical' }} />
              </div>

              <div style={fieldStyle}>
                <label style={labelStyle}>Allergies</label>
                <textarea className={inputClass} value={formData.allergies} onChange={setField('allergies')} placeholder="Allergies connues" rows={2} style={{ minHeight: 56, resize: 'vertical' }} />
              </div>

              <div style={fieldStyle}>
                <label style={labelStyle}>Médicaments actuels</label>
                <textarea className={inputClass} value={formData.current_medications} onChange={setField('current_medications')} placeholder="Traitements en cours" rows={2} style={{ minHeight: 56, resize: 'vertical' }} />
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, paddingTop: 8 }}>
                <button type="button" onClick={function() { setIsDialogOpen(false); }} style={{
      ...
      <div style={{
        ...
        {filteredPatients.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 0' }}>
            ...
            <h3 style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#475569', marginBottom: 4 }}>
          ...
          filteredPatients.map(function(patient, i) {
            return (
              <div
                key={patient.id}
                style={{
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    <h3 style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, fontSize: 15, color: '#0F172A', margin: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <button
                    onClick={function() { handleEdit(patient); }}
                    ...
                    onMouseEnter={function(e) { e.currentTarget.style.borderColor = '#3B4FD8'; e.currentTarget.style.color = '#3B4FD8'; e.currentTarget.style.background = '#EEF0FB'; }}
                    onMouseLeave={function(e) { e.currentTarget.style.borderColor = '#E2E8F0'; e.currentTarget.style.color = '#64748B'; e.currentTarget.style.background = '#fff'; }}
                  >
                    <Edit size={15} />
                  </button>
