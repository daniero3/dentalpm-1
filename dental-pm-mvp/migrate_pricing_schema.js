...
async function migrate() {
  ...
  try {
    ...
    // Get existing CABINET schedules
    const [cabinets] = await sequelize.query(`SELECT * FROM pricing_schedules WHERE type = 'CABINET'`, { transaction });
    console.log(`Found ${cabinets.length} CABINET schedules`);
    
    // Get existing CABINET fees
    const cabinetIds = cabinets.map(c => `'${c.id}'`).join(',');
    let cabinetFees = [];
    if (cabinetIds) {
      [cabinetFees] = await sequelize.query(`SELECT * FROM procedure_fees WHERE schedule_id IN (${cabinetIds})`, { transaction });
    }
    console.log(`Found ${cabinetFees.length} CABINET fees`);
    
    console.log('2) Dropping old tables...');
    await sequelize.query(`DROP TABLE IF EXISTS procedure_fees`, { transaction });
    await sequelize.query(`DROP TABLE IF EXISTS pricing_schedules`, { transaction });
    
    console.log('3) Creating new tables with clinic_id nullable...');
    ...
    // Create indexes
    await sequelize.query(`CREATE INDEX idx_ps_clinic_type ON pricing_schedules(clinic_id, type)`, { transaction });
    await sequelize.query(`CREATE INDEX idx_ps_clinic_active ON pricing_schedules(clinic_id, is_active)`, { transaction });
    await sequelize.query(`CREATE INDEX idx_pf_schedule ON procedure_fees(schedule_id)`, { transaction });
    await sequelize.query(`CREATE INDEX idx_pf_code ON procedure_fees(procedure_code)`, { transaction });
    
    console.log('4) Creating global SYNDICAL schedule...');
    ...
    `, { replacements: [syndicalId, now, now], transaction });
    
    console.log(`Created global SYNDICAL: ${syndicalId}`);
    
    console.log('5) Inserting SYNDICAL 2026 fees...');
    for (const fee of SYNDICAL_2026_FEES) {
      await sequelize.query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
      `, { replacements: [uuidv4(), syndicalId, fee.procedure_code, fee.label, fee.price_mga, fee.category || 'GENERAL', now, now], transaction });
    }
    console.log(`Inserted ${SYNDICAL_2026_FEES.length} SYNDICAL fees`);
    
    console.log('6) Restoring CABINET schedules...');
    ...
    console.log('7) Restoring CABINET fees...');
