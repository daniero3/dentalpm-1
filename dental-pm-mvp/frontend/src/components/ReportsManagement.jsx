  ...
  return (
    <div style={{ maxWidth: 1200,margin:'0 auto',paddingBottom:48 }}>
      ...
      <div style={{ background:'#fff',borderRadius:14,border:'1px solid #E2E8F0',padding:'14px 18px',marginBottom:20,display:'flex',gap:12,flexWrap:'wrap',alignItems:'flex-end' }}>
        <div>
          <label htmlFor="report-from-date" style={{ fontSize:11,fontWeight:600,color:'#64748B',display:'block',marginBottom:4,textTransform:'uppercase',letterSpacing:'.05em' }}>Du</label>
          <input id="report-from-date" aria-label="Date de début du rapport" type="date" value={fromDate} onChange={e=>setFromDate(e.target.value)} style={inp} onFocus={fi} onBlur={bi}/>
        </div>
        <div>
          <label htmlFor="report-to-date" style={{ fontSize:11,fontWeight:600,color:'#64748B',display:'block',marginBottom:4,textTransform:'uppercase',letterSpacing:'.05em' }}>Au</label>
          <input id="report-to-date" aria-label="Date de fin du rapport" type="date" value={toDate} onChange={e=>setToDate(e.target.value)} style={inp} onFocus={fi} onBlur={bi}/>
        </div>
        <button type="button" onClick={()=>fetchReport(fromDate, toDate)}
